...
async def main():
    # List semua file CSV yang tersedia
    csv_files = list(RAW_DIR.glob("*.csv"))
    if not csv_files:
        print("❌ Tidak ada file CSV di folder raw/")
        return
    ...
    try:
        choice = int(input("\nPilih nomor file yang ingin diproses (1-%d): " % len(csv_files)))
        if choice < 1 or choice > len(csv_files):
            print("❌ Nomor tidak valid")
            return
        
        selected_file = csv_files[choice - 1]
        print(f"\n🎯 File yang dipilih: {selected_file.name}")
